...
class AgenteSoporte(Agent):
    def __init__(self, chat_ctx=None, cliente_data: dict | None = None, evento_zona: str | None = None):
        ...
        _instructions += (
            ...
            "respóndele directamente usando los datos que ya tienes del cliente (balance, fecha_facturacion, fecha_suspension_automatica, costo del servicio). "
            "NO transfieras por este motivo. Usa endCall cuando el cliente quede satisfecho."
    ...
    async def llm_node(self, chat_ctx: llm.ChatContext, tools: list[llm.Tool], model_settings: ModelSettings):
        def _predicate(name: str | None) -> bool:
            if not name or not isinstance(name, str):
                return False
            n = name.strip()
            return n.endswith("end_call") or n.endswith("handoff_to_AgenteProspecto")
    ...
    async def llm_node(self, chat_ctx: llm.ChatContext, tools: list[llm.Tool], model_settings: ModelSettings):
        def _predicate(name: str | None) -> bool:
            if not name or not isinstance(name, str):
                return False
            n = name.strip()
            return n.endswith("end_call") or n.endswith("transferencia_llamada_warm")
        async for chunk in _suppress_text_llm_node(self, chat_ctx, tools, model_settings, _predicate):
            yield chunk
...
_RECEPCION_HANDOFF_TOOL_NAMES = frozenset(
    {"handoff_to_AgenteProspecto", "handoff_to_AgenteSoporte", "transferencia_llamada", "transferencia_llamada_warm", "end_call"}
)
    ...
    def __init__(self, *, identificado_por_sip: bool = False) -> None:
        ...
        super().__init__(
            instructions=(
                ...
                4. IF el cliente ya fue identificado en el paso 2 Y su intención es reportar alguna falla, lentitud, desconexiones, problemas con equipo, O pregunta por su servicio activo, paquete contratado, velocidad de su plan, estado de cuenta, pagos, facturación, tickets o cualquier detalle de su contrato actual (EXCEPTO los casos del paso 5):
