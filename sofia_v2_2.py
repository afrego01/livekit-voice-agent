...
class AgenteSoporte(Agent):
    def __init__(self, chat_ctx=None, cliente_data: dict | None = None, evento_zona: str | None = None):
        ...
        _instructions += (
            ...
            "necesitas transferirlo con uno de nuestros asesores, y usa endCall:"
        ...
        super().__init__(
            ...
            chat_ctx=chat_ctx,
            tools=[end_call, reiniciar_router, generar_ticket_soporte],
        )
...
_RECEPCION_HANDOFF_TOOL_NAMES = frozenset(
    {"handoff_to_AgenteProspecto", "handoff_to_AgenteSoporte"}
)


def _tool_name_is_recepcion_handoff(name: str | None) -> bool:
    """True si el tool call corresponde a un handoff de recepcionista.

    Coincidencia exacta o por sufijo: algunos proveedores/inference envían el nombre
    con prefijo o namespace distinto; el sufijo sigue siendo reconocible.
    """
    ...
    if n in _RECEPCION_HANDOFF_TOOL_NAMES:
        return True
    return n.endswith("handoff_to_AgenteProspecto") or n.endswith("handoff_to_AgenteSoporte")
...
class AgenteRecepcionista(Agent):
    def __init__(self, *, identificado_por_sip: bool = False) -> None:
        ...
        super().__init__(
            instructions=(
                ...
                4. IF la intención del cliente es reportar alguna falla, lentitud, desconexiones, problemas con equipo, O pregunta por su servicio activo, paquete contratado, velocidad de su plan, estado de cuenta, tickets o cualquier detalle de su contrato actual:

                4.1.  CALL inmediatamente ’handoff_to_AgenteSoporte’ sin decir nada más en ese turno (ni antes ni después de la herramienta).

                5. IF la intención es realizar un pago, información de facturación, cancelación, cambio de domicilio, seguimiento de visita técnica o hablar con una persona: usa endCall.
    ...
    async def handoff_to_AgenteProspecto(self, context: RunContext):
        ...
        # Solo el Agent (sin tupla ni string). En make_tool_output, si devuelves (Agent, ""),
        # el "" cuenta como output del tool y reply_required sigue True; el LLM puede generar
        # otra réplica. Con solo Agent, fnc_out queda None y el handoff no fuerza respuesta
        # adicional desde el resultado del tool. Ver: docs tool return / agents-handoffs.
...
async def entrypoint(ctx: JobContext):
    ...
    async def send_end_call_report():
        ...
        evento_zona = ud.get("evento_zona") or ""
        ...
        payload = {
            "cliente": {
                ...
                "nombre": cliente_data.get("nombre"),
                "celular": cliente_data.get("celular"),
                "estatus": cliente_data.get("estatus"),
            } if cliente_data.get("id") else None,
            "llamada": {
                "telefono_sip": ud.get("sip_celular_norm") or sip_raw,
                "identificado_por_sip": bool(ud.get("identificado_por_sip")),
                "afectacion_zona_activa": bool(evento_zona),
                "afectacion_zona_detalle": evento_zona or None,
                "inicio": inicio_llamada.isoformat(),
                "fin": fin_llamada.isoformat(),
                "duracion_segundos": duracion,
            },
            "metricas": {
                "tokens_prompt": getattr(summary, "prompt_tokens", None),
                "tokens_completion": getattr(summary, "completion_tokens", None),
                "tokens_total": getattr(summary, "total_tokens", None),
                "tts_caracteres": getattr(summary, "tts_characters_count", None),
            },
        }
